...
shinyServer(function(input, output) {
  ...
  draw.sample <- reactive({
    ...
    #N=length(x)
    #nvar=1
    #noise = nvar*rnorm(N,0,1)
    y = cos(x)+sin(x)
    fit <- krls(X=x, y=y, whichkernel = "gaussian", lambda=0.0325, sigma=0.0325, print.level = 0)
    model.summary <- rbind(fit$sigma, fit$lambda)
    rownames(model.summary) <- c("Sigma", "Lambda")
    colnames(model.summary) <- c("")
  ...
  regression <- reactive({
    ...
    if (a == 0.0325 & b == 0.0325) resid.color <- "seagreen" else resid.color <- "firebrick"
    
    # Calculate the current residuals
    model <- krls(X=x, y=y, whichkernel = "gaussian", lambda=a, sigma=b, print.level = 0)
    c <- model$fitted
    ...
    return(list(x=x, y=y, a=a, b=b, c=c, r2=r2,
  ...
  output$reg.plot <- renderPlot({         
    ...
    plot(x, c, type="l", lwd=10, col="red", lty=5,
         bty="n", xlim=c(-15, 15), ylim=c(-3, 8), xlab="x", ylab="y",
         main="")
